...
func createChildOrgWithTransitGateway(orgID, projectID, tgwID string, tgw model.TransitGateway) (*data.StructValue, error) {
	converter := bindings.NewTypeConverter()
	tgw.Id = &tgwID
...
func createChildOrgWithTransitGatewayMarkedForDelete(orgID, projectID, tgwID string) (*data.StructValue, error) {
	converter := bindings.NewTypeConverter()
	boolTrue := true
	tgw := model.TransitGateway{Id: &tgwID, ResourceType: strPtr("TransitGateway")}
	childTGW := model.ChildTransitGateway{
		Id:              &tgwID,
		ResourceType:    model.ChildTransitGateway__TYPE_IDENTIFIER,
...
func resourceNsxtPolicyTransitGatewayCreate(d *schema.ResourceData, m interface{}) error {
	...
	childOrg, err := createChildOrgWithTransitGateway(parents[0], parents[1], id, obj)
	if err != nil {
		return handleCreateError("TransitGateway", id, err)
	}
...
func resourceNsxtPolicyTransitGatewayDelete(d *schema.ResourceData, m interface{}) error {
	...
	childOrg, err := createChildOrgWithTransitGatewayMarkedForDelete(parents[0], parents[1], id)
	if err != nil {

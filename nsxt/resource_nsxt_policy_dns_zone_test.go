...
func testAccNsxtPolicyDnsZoneTemplate(createFlow bool) string {
	...
	return testAccNsxtDnsVnaClusterTemplate(attrMap["display_name"]) + fmt.Sprintf(`
resource "nsxt_policy_dns_service" "parent" {
  %s
  display_name           = "%s-svc"
  allocated_listener_ips = ["%s"]
  vns_clusters           = [nsxt_policy_virtual_network_appliance_cluster.vna.path]
  transit_gateway        = nsxt_policy_transit_gateway.dns_tgw.path
...
func testAccNsxtPolicyDnsZoneWithSoaTemplate() string {
	return testAccNsxtDnsVnaClusterTemplate(accTestPolicyDnsZoneCreateAttributes["display_name"]) + fmt.Sprintf(`
resource "nsxt_policy_dns_service" "parent" {
  %s
  display_name           = "%s-svc"
  allocated_listener_ips = ["%s"]

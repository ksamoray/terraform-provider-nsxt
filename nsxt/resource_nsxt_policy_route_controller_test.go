...
// testAccNsxtPolicyRouteControllerVnaTemplate returns HCL that creates a
// VirtualNetworkApplianceCluster and waits for its realization. The cluster
// resource is named "vna" so it does not clash with "test" resources in the
// same config.
...
resource "nsxt_policy_virtual_network_appliance_cluster" "vna" {
  display_name          = "tf-acc-rc-test-vna"
  appliance_form_factor = "MEDIUM"
  service_type          = "ROUTE_CONTROLLER"

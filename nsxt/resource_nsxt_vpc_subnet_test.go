...
func TestAccResourceNsxtVpcSubnet920_dnsServerPreference(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Providers: testAccProviders,
		CheckDestroy: func(state *terraform.State) error {
			return testAccNsxtVpcSubnetCheckDestroy(state, accTestVpcSubnetCreateAttributes["display_name"])
		},
		Steps: []resource.TestStep{
			{
...
}`, testAccNsxtPolicyMultitenancyContext(), accTestVpcSubnetCreateAttributes["display_name"], accTestVpcSubnetCreateAttributes["description"], accTestVpcSubnetCreateAttributes["ipv4_subnet_size"], preference)
}

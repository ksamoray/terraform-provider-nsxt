...
func getPolicyIPAssignmentSchema(required bool, minItems, maxItems int) *schema.Schema {
	return &schema.Schema{
		...
		Elem: &schema.Resource{
			Schema: map[string]*schema.Schema{
				...
				"static_ipv4_pool": {
					Type:        schema.TypeString,
					Description: "IP assignment specification for Static IPv4 Pool. Input can be MP ip pool UUID or policy path of IP pool",
					Optional:    true,
				},
				"static_ipv6": {
					Type:        schema.TypeList,
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"management_interface": {
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"ip_assignment": getPolicyIPAssignmentSchema(true, 1, 2),
						"network_id": {
							Type:        schema.TypeString,
							Required:    true,
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"overlay_transport_zone_path": {
							Type:        schema.TypeString,
							Optional:    true,
							Description: "An overlay TransportZone path that is associated with the specified edge TN switch",
						},
						"pnic": {
							Type:        schema.TypeList,
							Required:    true,
						...
						"tunnel_endpoint": {
							...
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									"ip_assignment": getPolicyIPAssignmentSchema(true, 1, 1),
									"vlan": {
										Type:         schema.TypeInt,
										Optional:     true,
...
func getVMDeploymentConfigFromSchema(iVmDeploymentCfg interface{}) (*data.StructValue, error) {
	...
	for _, iDeploymentCfg := range iVmDeploymentCfg.([]interface{}) {
		...
		vcID := deploymentCfg["vc_id"].(string)

		vmDeploymentCfg := model.PolicyVsphereDeploymentConfig{
			ComputeFolderId:        &computeFolderId,
			ComputeId:              &computeId,
...
func setCredentialsInSchema(d *schema.ResourceData, obj *model.PolicyEdgeTransportNodeCredential) error {
	...
	creds["cli_password"] = obj.CliPassword
	creds["cli_username"] = obj.CliUsername
	creds["root_password"] = obj.RootPassword
...
func resourceNsxtPolicyEdgeTransportNodeRead(d *schema.ResourceData, m interface{}) error {
	...
	if obj.VmDeploymentConfig != nil {
		...
		if base.(model.PolicyVmDeploymentConfig).PlacementType == model.PolicyVmDeploymentConfig_PLACEMENT_TYPE_POLICYVSPHEREDEPLOYMENTCONFIG {
			...
			depCfg["vc_id"] = deploymentConfig.VcId
		}
	}

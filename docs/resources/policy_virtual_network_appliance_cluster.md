...
* `password_managed_by_vcf` - (Optional) When set to `true`, enables VCF password management for all virtual network appliances in the cluster.
...
* `advanced_configuration` - (Optional) Advanced configuration for virtual network appliances in the cluster. This block supports:
    * `ha_over_management_interface` - (Optional) Enable or disable synchronization of high-availability states between VNA nodes over the management interface within the cluster.
    * `high_availability_profile` - (Optional) Path to the high availability profile. If not provided, the default profile is used.

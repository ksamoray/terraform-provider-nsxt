...
* `advanced_configuration` - (Optional) Advanced configuration for virtual network appliances in the cluster. This block supports:
    ...
    * `high_availability_profile` - (Optional, Computed) Path to the high availability profile. If not specified, NSX assigns a default profile. If not provided, the default profile is used.

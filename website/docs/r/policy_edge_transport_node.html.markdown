...
* `management_interface` - (Optional) Applicable For LCM managed Node and contains the management interface info.
  * `ip_assignment` - (Required) IPv4 or Ipv6 Port subnets for management port.
    * `auto_conf` - (Optional) Enable Auto-conf based IPv6 assignment.
    * `dhcp_v4` - (Optional) Enable DHCP based IPv4 assignment.
    * `dhcp_v6` - (Optional) Enable DHCP based IPv6 assignment.
    * `no_assignment` - (Optional) Enable no IP assignment.
    ...
    * `static_ipv4_mac_list` - (Optional) IP and MAC assignment specification for Static IP List.
      * `default_gateway` - (Required) Gateway IP.
      * `ip_mac_pair` - (Required) List of IP MAC pairs.
        * `ip_address` - (Required) A single IPv4 address.
        * `A single MAC address` - (Required) A single MAC address.
        * `subnet_mask` - (Required) Subnet mask.
    ...
    * `static_ipv6_mac_list` - (Optional) IP and MAC assignment specification for Static IPv6 List.
      * `default_gateway` - (Required) Gateway IP.
      * `ip_mac_pair` - (Required) List of IP MAC pairs.
        * `ip_address` - (Required) A single IPv6 address.
        * `A single MAC address` - (Required) A single MAC address.
      * `prefix_length` - (Required) Prefix Length.
